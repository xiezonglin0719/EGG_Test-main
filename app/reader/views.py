...
from . import reader
from .forms import SearchBookForm, AdminUserForm
...
# ------------------------------ 渲染页面路由 --------------------------------#
# 渲染读者首页
@reader.route('/signal', methods=['GET', 'POST'])
@login_required
def index():
    form = SearchBookForm()
    return render_template('function/signal.html', form=form)


 #渲染读者个人信息页面
@reader.route('/information', methods=['GET', 'POST'])
@login_required
def user():
    form = AdminUserForm()
    return render_template('function/information.html', form=form)


# 渲染读者提交想看图书列表页面
@reader.route('/evaluate', methods=['GET', 'POST'])
@login_required
def wantsbook():
    return render_template('function/evaluate.html')
